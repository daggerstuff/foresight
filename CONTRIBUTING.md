...
1. **No error suppression.** No `# noqa` to silence real findings, no
   `# type: ignore` to dodge the checker, no loosening the ruff/pyright config
   to make a failing gate pass. Fix the cause.
2. **Tenant scope everywhere.** Any new store/read path filters on `tenant_id` +
   `user_id`. If you add a storage API, add a scope-isolation test next to
...
4. **SQLite is for tests; Postgres is for prod.** Don't add SQLite-only SQL to
   production paths.
5. **Match the house style.** Lazy imports in the server hot path, singleton
   accessors via module-level `get_*()` functions, pydantic validation at tool
   boundaries. See [ARCHITECTURE.md](ARCHITECTURE.md) for the map.

## CI

Every push and PR runs three gates
...
- New tools in `server.py` need: schema validation, tenant scoping, tests, and a
  docstring — agents read those docstrings as their only manual.
