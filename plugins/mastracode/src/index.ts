...
const DEFAULT_FORESIGHT_URL = process.env.FORESIGHT_MCP_URL ?? 'http://127.0.0.1:8764';
...
export async function autoCaptureTurn(
  ...
  if (assistantMessage) {
    messages.push({ role: 'assistant', content: assistantMessage.slice(0, 2000) });
  }
...
export function createForesightProcessor(config?: ForesightClientConfig): InputProcessor & OutputProcessor {
  return {
    id: 'foresight-context-processor',
    name: 'Foresight Context & Capture Processor',
    description: 'Zero-touch continuity context injection and background turn capture.',

    async processInput(args: any) {
      const { messages, systemMessages, requestContext } = args;
      const threadId = requestContext?.threadId || requestContext?.sessionId || 'mastracode-session';
      ...
      const hasDirectives = sysList.some(
        (m) => (typeof m === 'string' && m.includes('Foresight Persistent Memory Directives')) ||
               (typeof m?.content === 'string' && m.content.includes('Foresight Persistent Memory Directives'))
      );
    ...
    async processOutputResult(args: any) {
      ...
      if (result && typeof result.text === 'string' && result.text.trim()) {
        ...
        if (last && (last.role === 'assistant' || last.sender === 'assistant')) {
...
export function createForesightTools(config?: ForesightClientConfig) {
  const injectContextTool = createTool({
    ...
    execute: async (input: any) => {
      ...
      const max_memories = input?.max_memories ?? input?.context?.max_memories ?? 6;
  ...
  const storeMemoryTool = createTool({
    ...
    execute: async (input: any) => {
      ...
      const retention = input?.retention ?? input?.context?.retention ?? 'permanent';
...
export default defineMastraCodePlugin({
  ...
  version: '1.0.0',
  description: 'Hands-off persistent memory, continuity context injection, and background distillation for Mastra Code.',

  config: {
    serverUrl: {
      type: 'string',
      label: 'Foresight Server URL',
      description: 'Base URL for the Foresight server or MCP endpoint (e.g. http://127.0.0.1:8764)',
      default: 'http://127.0.0.1:8764',
    },
    userId: {
      type: 'string',
      label: 'User ID',
      description: 'User identifier for tenant/user isolation (e.g. default or your username)',
      default: 'default',
    },
    autoInject: {
      type: 'boolean',
      label: 'Auto-inject Context',
      description: 'Automatically inject relevant memories and active context blocks before each turn',
      default: true,
    },
    autoCapture: {
      type: 'boolean',
      label: 'Auto-capture Turns',
      description: 'Automatically distill facts, decisions, and preferences from completed turns in background',
      default: true,
    },
  },
  ...
  tools: (context: MastraCodePluginContext) => {
    const clientConfig: ForesightClientConfig = {
      baseUrl: (context.config.serverUrl as string) || process.env.FORESIGHT_MCP_URL || 'http://127.0.0.1:8764',
