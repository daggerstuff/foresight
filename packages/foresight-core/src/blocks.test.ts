...
describe("BlockManager", () => {
  ...
  it("rejects duplicate schemas and unknown block labels", () => {
    ...
    expect(() => manager.createBlock("missing")).toThrow("Block schema 'missing' not found");
  });
  ...
  it("clears blocks without removing registered schemas", () => {
    ...
    expect(manager.getSchema("session_patterns")?.label).toBe("session_patterns");
  });
});
...
describe("MemoryInjector", () => {
  ...
  it("truncates content exceeding charLimit", () => {
    ...
    manager.createBlock("strict", "This is a very long guidance text that should be truncated");
